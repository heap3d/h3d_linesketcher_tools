...
def get_resolution_override(name=""):
    ...
    try:
        ...
        print(
            "resolution override error: <{}>, default used, UV Map: <{}>   start: <{}>   end: <{}>".format(
                error.message, name, start, end
            )
        )
        return imgSize_default
...
def generateMaterials(mesh):
    ...
    mask_2 = scene.addItem(
        "mask", "%s bc_tr *RGB* --- metal: diffuse=0 spec=100" % pass_2
    )
    ...
    grad_diffAmountScan.channel(
        "color.R"
    ).envelope.postBehaviour = lx.symbol.iENV_LINEAR
    ...
    lx.eval(
        'clip.newStill "%s%s.png" x%s RGB false false format:PNG'
        % (newImagePath, imgName_ao_ro_me, imgSize_override)
    )
    ...
    lx.eval(
        'clip.newStill "%s%s.png" x%s RGB false false format:PNG'
        % (newImagePath, imgName_norm, imgSize_override)
    )
    ...
    lx.eval(
        'clip.newStill "%s%s.png" x%s RGB false false format:PNG'
        % (newImagePath, imgName_lum, imgSize_override)
    )
    ...
    lx.eval(
        'clip.newStill "%s%s.png" x%s RGB false false format:PNG'
        % (newImagePath_tmp, imgName_tr_tmp, imgSize_override)
    )
    ...
    lx.eval(
        'clip.newStill "%s%s.png" x%s RGB false false format:PNG'
        % (newImagePath_tmp, imgName_me_fl, imgSize_override)
    )
    ...
    lx.eval(
        'clip.newStill "%s%s.png" x%s RGB false false format:PNG'
        % (newImagePath_tmp, imgName_di_di, imgSize_override)
    )
    ...
    lx.eval(
        'clip.newStill "%s%s.png" x%s RGB false false format:PNG'
        % (newImagePath_tmp, imgName_me_sp, imgSize_override)
    )
    ...
    lx.eval(
        'clip.newStill "%s%s.png" x%s RGB false false format:PNG'
        % (newImagePath_tmp, imgName_bc_tmp, imgSize_override)
    )
...
def getTodoList():
    ...
    if lpItem is None:
        ...
        print("cancelled")
        modo.dialogs.alert(
            title="cancelled", message="lowpoly item not found", dtype="warning"
        )
        exit()
...
bake_selected = lx.eval("user.value {%s} ?" % userValSelectedName) != 0
bakeOn = lx.eval("user.value {%s} ?" % userValBakeName) != 0
...
for item in meshByUvList:
    ...
    lx.eval(
        "select.subItem {%s:videoStill001} set textureLayer;mediaClip" % videoClip.name
    )
    if bakeOn:
        lx.eval("clip.save")
    # lx.eval('!clip.reload')
...
for mask in scene.items("mask"):
    ...
    if mask.name[: len(mask_search)] == mask_search and is_item_mask:
        ...
        lx.eval(
            'select.subItem "%s" set textureLayer;render;environment;light;camera;scene;replicator;bake;mediaClip;\
                txtrLocator'
            % mask.id
        )
