...
def newTexture(
    ...
    imgName = newName
    lx.eval(
        'clip.newStill "%s\\%s%s" x%s RGB false false format:%s colorspace:%s'
        % (newImagePath, imgName, newExt, imgSize, imgFormat, createSpace)
    )
...
# gltf material initialization
mat_gltf = None
# gltf ambient occlusion map initialization
map_ao = None
# gltf normal map initialization
map_norm = None
# gltf emission map initialization
map_lum = None
# gltf base color map initialization
map_bc = None
# gltf transparent amount map initialization
map_tr = None
# gltf metallic map initialization
map_me = None
# 1st pass map initialization
map_pass1 = None
# 2nd pass map initialization
map_pass2 = None
# 3rd pass map initialization
map_pass3 = None
...
for mask in selectedMasks:
    ...
    mat_gltf = (
        ...
        map_norm
    ) = map_lum = map_bc = map_tr = map_me = map_pass1 = map_pass2 = map_pass3 = None
    ...
    # skip if map_ao or map_me not found
    except Exception:
    ...
    folder_path, map_bc_filename = os.path.split(
        map_ao_copy.itemGraph("shadeLoc").forward(1).channel("filename").get()
    )
    ...
    # get new map name
    map_bc_ao_merged_filename = "{}__bc_ao_merged".format(
        map_bc_filename[: map_bc_filename.rfind("__")]
    )
    ...
    map_bc_ao_merged = newTexture(
        ...
        newName=map_bc_ao_merged_filename,
        newExt=image_ext,
        newImagePath=folder_path,
        uvMapName=map_bc_uvmap_name,
        effect="baseColorGLTF",
        createSpace="(default)",
        loadSpace="(default)",
    )  # type: modo.item.Item
...
try:
    ...
    modo.dialogs.alert(
        title="cancelled", message="no baked group found", dtype="warning"
    )
